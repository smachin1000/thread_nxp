...
uint32_t IP_EnetOpen(ifHandle_t* pIfHandle);
uint32_t IP_EnetClose(ifHandle_t* pIfHandle);
...
uint32_t IP_EnetJoin(ifHandle_t* pIfHandle, ipAddr_t* pIpAddr,uint16_t protocol);
uint32_t IP_EnetLeave(ifHandle_t* pIfHandle, ipAddr_t* pIpAddr,uint16_t protocol);

#if IP_IP4_ENABLE
uint32_t IP_EnetOpen4(ifHandle_t* pIfHandle);
...
uint32_t IP_EnetOpen6(ifHandle_t* pIfHandle);
uint32_t IP_EnetGetIID(ifHandle_t* pIfHandle,llAddr_t* macAddr,ipAddr_t* ipAddr);
#endif
...
static const uint8_t* mIfName = "eth";
...
{ 
    ...
    enetHdr_t* enetpkt = (enetHdr_t*)pInData;
    uint16_t protocol = ntohas(enetpkt->type);
    ifHandle_t pIfHandle = *(ifHandle_t*)privateData; 
    ...
    pIpPktInfo->pIpPktOptions->ifHandle = privateData;
    ...
    pIpPktInfo->pNwkBuff->freeBuffer = 0;
    
    pIpPktInfo->pNextProt = pInData + ENET_HEADER_SIZE;
...
#if IP_IP6_ENABLE
/*!*************************************************************************************************
\fn uint32_t IP_EnetOpen6(ifHandle_t* pIfHandle)
...
)
{
    ...
    /* Register IP6 */
    error = ENET_open((*pIfHandle)->ifDriverHandle,gIpProtv6_c,IP_EnetRecv,(void*)pIfHandle);
...
#if IP_IP4_ENABLE
/*!*************************************************************************************************
\fn uint32_t IP_EnetOpen4(ifHandle_t* pIfHandle)
...
)
{
    uint32_t  error = gIpGeneralError_c;
    /* Register IP4 */
    error = ENET_open((*pIfHandle)->ifDriverHandle,gIpProtv4_c,IP_EnetRecv,(void*)pIfHandle);

    if(error == gIpOk_c)
    {
        error = ENET_open((*pIfHandle)->ifDriverHandle,gIpProtArp_c,IP_EnetRecv,(void*)pIfHandle);    
...
==================================================================================================*/
/*!*************************************************************************************************
\fn uint32_t IP_EnetOpen(ifHandle_t* pIfHandle)
...
)
{
    ...
    (*pIfHandle)->ifMtu = ENET_get_MTU((*pIfHandle)->ifDriverHandle);
    (*pIfHandle)->ifDevAddrTbl[0].addrSize = gLlayerAddrEui48_c;
    
    ENET_get_address((*pIfHandle)->ifDriverHandle,(*pIfHandle)->ifDevAddrTbl[0].eui);

#if IP_IP4_ENABLE 
    if((*pIfHandle)->ipVersion4)
    {
        error1 = IP_EnetOpen4(pIfHandle);
    }
#endif
    ...
    {
        error2 = IP_EnetOpen6(pIfHandle);
    }        
#endif
    
    if((error1 != gIpOk_c) || (error2 != gIpOk_c))
    {
        IP_EnetClose(pIfHandle);
    }
    return error1 + error2;
}
/*!*************************************************************************************************
\fn uint32_t IP_EnetClose(ifHandle_t* pIfHandle)
...
/*!*************************************************************************************************
\fn uint32_t IP_EnetGetIID(ifHandle_t* pIfHandle)
...
#endif
/*!*************************************************************************************************
\fn uint32_t IP_EnetJoin6(ifHandle_t* pIfHandle, ipAddr_t* pIpAddr,uint16_t protocol)
...
)
{
    ...
    return ENET_join((*pIfHandle)->ifDriverHandle,enetGroupAddr,protocol);
}   
/*!*************************************************************************************************
