...
#if MQX_ALLOCATOR_GARBAGE_COLLECTING
    /*
     * Move ownership of the td structure from the creating task, to the
     * newly created task.
     */
    if (input_stack_ptr == 0)
    {
        _mem_transfer(td_ptr, creator_task_id, td_ptr->TASK_ID);
        _mem_transfer(td_ptr->STACK_ALLOC_BLOCK, creator_task_id, td_ptr->TASK_ID);
    } /* Endif */
#endif /* MQX_ALLOCATOR_GARBAGE_COLLECTING */
...
)
{
    ...
    td_ptr = _task_build_internal(
        0, (uint32_t)&(temp_template), taskinit_p->stackaddr, taskinit_p->stacksize, FALSE
    );
