...
const uint32_t          _mqx_version_number = 0x05000001;
const char       *const _mqx_vendor         = "Freescale/Freescale MQX";

/* Identify the product */
#if MQX_CRIPPLED_EVALUATION
const char      *const _mqx_version        = "5.0.1c";
#else
const char      *const _mqx_version        = "5.0.1";
#endif
...
#       else /* PSP_STACK_ALIGNMENT */
        result = kernel_data->INIT.INTERRUPT_STACK_SIZE;
#       endif /* PSP_STACK_ALIGNMENT */
...
#if MQX_USE_INTERRUPTS
#if MQXCFG_PREALLOCATED_SYSTEM_STACKS == 1
	/* Now obtain the interrupt stack */
	stack_ptr = mqx_interrupt_stack;
	result = INTERRUPT_STACK_SIZE;
	_mem_set_type(stack_ptr, MEM_TYPE_INTERRUPT_STACK);
#if MQX_MONITOR_STACK
    _task_fill_stack_internal((_mqx_uint_ptr)stack_ptr, result);
#endif /* MQX_MONITOR_STACK */
