...
static void insert_free_block(control_t* control, block_header_t* block, int fl, int sl)
{
    ...
    tlsf_assert(((tlsfptr_t)block_to_ptr(block) + (tlsfptr_t)block_header_overhead) == (tlsfptr_t)align_ptr((tlsfptr_t)block_to_ptr(block) + \
    (tlsfptr_t)block_header_overhead, ALIGN_SIZE) && "block not aligned properly");
 ...
 * \cond DOXYGEN_PRIVATE
 */
void* tlsf_memalign(tlsf_t tlsf, size_t align, size_t size, size_t bytes_to_aligned)
{
    control_t* control = tlsf_cast(control_t*, tlsf);
    const size_t adjust = adjust_request_size(size, ALIGN_SIZE);
    ...
    */
    ...
    const size_t aligned_size = (align <= ALIGN_SIZE) ? adjust : size_with_gap;
        ...
        tlsf_assert((size !=0) && ((align&(~(align-1)))==align) && "Alignment must be power of 2 and size cannot be zero!");
...
void* tlsf_realloc_aligned(tlsf_t tlsf, void* ptr, size_t size, size_t alignment, size_t bytes_to_aligned)
{
    ...
    #endif	/* else: handled by higher layer*/
    {
        ...
        */
        if (adjust > cursize && (!block_is_free(next) || adjust > combined))
        {
                        p = tlsf_memalign(tlsf, alignment, size, bytes_to_aligned);
            if (p)
            {
                const size_t minsize = tlsf_min(cursize, size);
                memcpy(p, ptr, minsize);
