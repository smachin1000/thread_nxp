            ...
            if (NULL != dev) {
                ...
                if (0 == res) {
                    ...
                    if (res) {
                        ...
                        if (dev->FN->DEINIT)
                        {
                            error = dev->FN->DEINIT(&dev->CONTEXT);
                            assert(!error); //deinit failed, device driver problem
...
static int _nio_open_intern(NIO_DEV_STRUCT *dev, const char *name, int flags, int mode) {
    ...
    if (NULL != dev) {
        if (NULL != dev->FN->OPEN) {
            ...
            err = dev->FN->OPEN(dev->CONTEXT, name, flags, &fp_context);
            if (err) {
                // error, exception
                // free resourcess
                lock_fd_table();
...
int _nio_close(int fd) {
    ...
    if (NULL != dev) {
        ...
        if (NULL != dev->FN->CLOSE) {
            err = dev->FN->CLOSE(dev->CONTEXT, fp->CONTEXT);
            if (err) {
                // error, exception
                errno = err;
            }
...
_nio_off_t _nio_lseek(int fd, _nio_off_t offset, int whence) {
    _nio_off_t res = -1;
    NIO_FILE *fp;
    NIO_DEV_STRUCT *dev;
